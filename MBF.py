	...
	while 1:
		...
		if v:
			if a.upper() in v:
				...
				tampil('\rm[!]Masukan Opsinya BangSat!!...')
				continue
		else:
			if len(a) == 0:
...
def login():
	global log
	us = inputD('[?]Email/HP')
	pa = inputD('[?]!Sorry Ngegass [*]Kata Sandi')
	...
	if 'save-device' in url or 'm_sess' in url:
		...
		tampil('\rm[!]Login Gagal, Mungkin Lo Jelek!')
...
def saring_id_group0():
	...
	while 1:
		...
		try:
			...
			tampil('\rm[!]Id yang anda masukan salah goblok, hehe..')
			continue
...
def menu():
	tampil('''\rh
            ...
            \rk*   *\rh /+/++//
             \ /  |/__//
           {\rmX\rh}v{\rmO\rh}|\rcAAN\rh|==========.
             [']  /'|'\           \\
                 /  \  \           '
                 \_  \_ \_    \rk*\rhCtrX Squad
\rk###########################################################
#              \rb*HACK TOOLSS BY CtrX*\rk                      #
# \rhBY\rp                                           ALpian9890 \rk#
# \rhInstagram\rp                                   @alpian9890 \rk#
# \rhGitHub\rp                    https://github.com/alpian9890 \rk#
#          \rmA child is prohibited from using this          \rk#
###########################################################''')
